...
int main(void)
{
	...
	//JSON_Array *arrayValue = json_value_get_array(rootValue);
    ...
    for (int i = 0 ; i < arrayCount ; i++)
    {
		...
		printf("threadOb : %s\n", json_object_get_string(threadOb, "name"));
 / 		pthread_create(&thread[i],&attr,thread_routine(NULL, threadName), NULL);
    }
